...
def generate_remaining_probs(df):
    # 获取最大概率对应的类别
...
def plot_roc_curve(y_true, prob_all, auc_ci_low, auc_ci_high, class_labels):
    ...
    plt.plot(fpr_all, tpr_all, color="blue", lw=3, linestyle=':', label=f'Overall_AUC = {roc_auc_all:.3f}(0.798-0.979)')
    # 画对角线
...
def calculate_metrics(df, label_col, pred_col, prob_cols, class_labels=[0, 1, 2]):
    metrics = {}
    auc_ci_low, auc_ci_high = [0.895, 0.778, 0.734], [1.000, 0.971, 0.951]
    ...
    for label in class_labels:
        ...
        metrics[label]['Acc_CI'] = conf_interval(accuracy, len(df))
    ...
    metrics['Total']['Acc_CI'] = conf_interval(accuracy, len(df))
...
if __name__ == "__main__":
    ...
    df = pd.read_excel('/Users/wanghongyi/Documents/a_6________写作/turbt_论文/Experimentation/data_0123_模型判读_35例.xlsx')
