...
def plot_roc_curve(y_true_all_binarized, prob_all, auc_ci_low, auc_ci_high, class_labels):
    ...
    save_path = os.path.join(save_dir, f"低级别与高级别癌_内部验证_ROC.pdf")
...
def plot_confusion_matrix(cm, class_label, title='Confusion Matrix'):
    ...
    save_dir = '/Volumes/WHY-SSD/trubt_paper_pics/验证'
    os.makedirs(save_dir, exist_ok=True)  # 确保目录存在
    save_path = os.path.join(save_dir, f"低级别与高级别癌_内部验证_confusionMatrix.pdf")
    plt.savefig(save_path, format='pdf', dpi=300, bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    df = pd.read_excel('/Users/wanghongyi/Documents/a_6________写作/turbt_论文/Experimentation/副本模型验证结果+概率-250414.xlsx', sheet_name='内部验证-高低级别')
