...
def load_data(file_path):
    ...
    data = pd.read_excel(file_path, sheet_name='1015-高低级别')
    labels = data['Label'].values
    predictions = data['Pred'].values
    probabilities = data[['Prob_Cls0', 'Prob_Cls1', 'Prob_Cls2']].values
...
def calculate_metrics(labels, predictions, probabilities, n_bootstrap=1000):
    ...
    # 打印总体指标和置信区间
    print(
        f"Overall AUC: {overall_auc:.3f}, 95% CI: ({overall_auc - ci_overall_auc:.3f}, {overall_auc + ci_overall_auc:.3f})")
    print(f"Overall Sensitivity: {overall_sensitivity:.3f}, 95% CI: {ci_overall_sensitivity}")
    print(f"Overall Specificity: {overall_specificity:.3f}, 95% CI: {ci_overall_specificity}")
    print(f"Overall PPV: {overall_ppv:.3f}, 95% CI: {ci_overall_ppv}")
    print(f"Overall NPV: {overall_npvs:.3f}, 95% CI: {ci_overall_npvs}")
    print(f"Overall ACC: {overall_accs:.3f}, 95% CI: {ci_overall_accs}")
...
def plot_roc_curve(labels, probabilities, class_names, auc_ci_low, auc_ci_high, font_size=12):
    ...
    save_path = os.path.join(save_dir, f"低级别与高级别癌_ROC.pdf")
...
def plot_confusion_matrix(labels, predictions, class_names, font_size=12):
    ...
    save_dir = 'H:\\trubt_paper_pics\\人工判读与算法判读对比\\1015例'
    os.makedirs(save_dir, exist_ok=True)  # 确保目录存在
    save_path = os.path.join(save_dir, f"低级别与高级别癌_confusionMatrix.pdf")
    plt.savefig(save_path, format='pdf', dpi=300, bbox_inches='tight')
...
def main(file_path):
    ...
    # 绘制混淆矩阵
    class_names = ['Non-tumor', 'Low-grade', 'High-grade']
    ...
    # 打印AUC和其他指标
    print("AUC, Sensitivity, Specificity, PPV, NPV per class:")
    # 打印每个类别的AUC、Sensitivity、Specificity、PPV、NPV及其置信区间
    auc_ci_low = []
    auc_ci_high = []
    for i in range(3):
        print(f"Class {i}:")
        print(f"AUC: {metrics['AUC'][i]:.3f}, 95% CI: {ci_metrics['AUC_CI'][i]}")
        print(f"Sensitivity: {metrics['Sensitivity'][i]:.3f} {ci_metrics['Sensitivity_CI'][i]}")
        print(f"Specificity: {metrics['Specificity'][i]:.3f} {ci_metrics['Specificity_CI'][i]}")
        print(f"PPV: {metrics['PPV'][i]:.3f} {ci_metrics['PPV_CI'][i]}")
        print(f"NPV: {metrics['NPV'][i]:.3f} {ci_metrics['NPV_CI'][i]}")
        print(f"ACC: {metrics['ACC'][i]:.3f} {ci_metrics['ACC_CI'][i]}")
        auc_ci_low.append(ci_metrics['AUC_CI'][i][0])
        auc_ci_high.append(ci_metrics['AUC_CI'][i][1])
    print(auc_ci_low, auc_ci_high)
    # 绘制ROC曲线
